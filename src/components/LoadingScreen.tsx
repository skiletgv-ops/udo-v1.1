...
import { Presentation, Play, Film, Sparkles } from 'lucide-react';
...
export function LoadingScreen({ onComplete }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Pause progress while presentation modal is open
  ...
  }, [onComplete, showPresentationModal]);
      ...
      <AnimatePresence>
        {!isFadingOut && (
          <motion.div
            ...
            className="fixed inset-0 z-40 flex flex-col items-center justify-center bg-[#0a0a0f]/90 backdrop-blur-md px-4"
          >
            ...
            <div className="w-80 max-w-full space-y-2 mb-8">
            ...
            {/* WATCH UDO PRESENTATION BUTTON */}
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => setShowPresentationModal(true)}
              className="px-6 py-3 rounded-2xl bg-gradient-to-r from-cyan-500/20 via-blue-600/30 to-cyan-500/20 border border-cyan-400/50 hover:border-cyan-400 text-cyan-300 hover:text-white font-mono text-xs font-bold uppercase tracking-wider shadow-[0_0_25px_rgba(6,182,212,0.3)] hover:shadow-[0_0_40px_rgba(6,182,212,0.6)] transition-all cursor-pointer flex items-center gap-2.5 z-50"
            >
              <Presentation className="w-4 h-4 text-cyan-400 animate-pulse" />
              <span>WATCH UDO PRESENTATION 🎬</span>
            </motion.button>
          </motion.div>
        )}
      </AnimatePresence>

      {/* UDO PRESENTATION VIDEO & DECK MODAL */}
