...
export const BottomDock: React.FC<BottomDockProps> = ({
  ...
  const dockItems: { id: ActiveTab | null; label: string; icon: React.ReactNode; title: string; badge?: number }[] = [
    ...
    { id: 'upload', label: isDe ? 'Akte Upload' : 'Patient Upload', icon: <FolderUp className="w-5 h-5 text-cyan-300 shrink-0" />, title: isDe ? 'Patientenakte & Import' : 'Patient Upload & DICOM' },
    { id: 'scan', label: isDe ? 'KI-Scan' : 'AI Scan', icon: <Sparkles className="w-5 h-5 text-cyan-400 shrink-0" />, title: isDe ? '4-KI-Agenten S2k Scan' : '4-AI Agent Scan' },
    { id: 'review', label: isDe ? 'Befunde' : 'Findings', icon: <Eye className="w-5 h-5 text-teal-300 shrink-0" />, title: isDe ? 'Befund-Bewertung & Review' : 'Findings Review' },
    { id: 'gutachten', label: isDe ? 'Gutachten' : 'Reports', icon: <FileText className="w-5 h-5 text-emerald-400 shrink-0" />, title: isDe ? 'S2k Gutachten Generator' : 'S2k Forensic Reports' },
    { id: 'consult', label: isDe ? 'Consult' : 'Consult', icon: <Mic className="w-5 h-5 text-rose-400 shrink-0 animate-pulse" />, title: isDe ? 'Voice & Konsil' : 'Voice Consult' },
    { id: 'eeg', label: isDe ? 'EEG Core' : 'EEG Core', icon: <Activity className="w-5 h-5 text-indigo-400 shrink-0" />, title: isDe ? 'EEG Signalverarbeitung' : 'EEG Workspace' },
    { id: 'video', label: isDe ? 'Video' : 'Video', icon: <Video className="w-5 h-5 text-violet-400 shrink-0" />, title: isDe ? 'Video-Analyse & Biometrie' : 'Video Gait Diagnostics' },
    { id: 'documents', label: isDe ? 'Dokumente' : 'Documents', icon: <FolderOpen className="w-5 h-5 text-cyan-400 shrink-0" />, title: isDe ? 'Dokumente & OCR Pipeline' : 'Documents & OCR' },
    ...
    { id: 'dictate', label: isDe ? 'Diktat' : 'Dictate', icon: <Mic className="w-5 h-5 text-violet-400 shrink-0" />, title: isDe ? 'Sprach-Diktat' : 'Voice Dictation' },
    { id: 'insurance', label: isDe ? 'Kosten' : 'Costs', icon: <FileCheck2 className="w-5 h-5 text-cyan-300 shrink-0" />, title: isDe ? 'Kostenübernahme & Kasse' : 'Costs & Insurance' },
    { id: 'analytics', label: isDe ? 'Statistik' : 'Analytics', icon: <BarChart3 className="w-5 h-5 text-amber-400 shrink-0" />, title: isDe ? 'Praxis Performance & KPIs' : 'Practice Analytics' },
    { id: 'retention', label: isDe ? 'DSGVO' : 'GDPR', icon: <Lock className="w-5 h-5 text-rose-400 shrink-0" />, title: isDe ? 'Fristen & Retention (10J)' : 'GDPR Retention' },
      ...
      ? [
          {
            ...
            label: isDe ? 'Genehmigung' : 'Approvals',
            icon: <ShieldCheck className="w-5 h-5 text-emerald-400 shrink-0" />,
            title: isDe ? 'QES Rezept-Freigabe' : 'QES Recipe Approvals',
            badge: pendingCount
          }
        ]
      : [])
